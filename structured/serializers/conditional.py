...
from .api import Serializer, ByteOrder
from ..type_checking import Ts, Unpack, Generic, Callable, TYPE_CHECKING, TypeVar, Any, WritableBuffer, ReadableBuffer, BinaryIO, Self
...
class ConditionalSerializer(Generic[Unpack[Ts]], Serializer[Unpack[Ts]]):
    def __init__(self, serializer: Serializer[Unpack[Ts]], condition: Callable[[TStructured], bool], default: tuple[Unpack[Ts]]) -> None:
        ...
        if serializer.num_values != len(default):
            raise ValueError('Not enough default arguments')
...
class SkipSerializer(Generic[Unpack[Ts]], Serializer[Unpack[Ts]]):
    ...
    def pack_into(self, buffer: WritableBuffer, offset: int, *values: Unpack[Ts]) -> None:
        pass
